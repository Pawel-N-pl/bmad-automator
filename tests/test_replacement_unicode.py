...
class StateBuildUnicodeTests(_FixtureMixin, unittest.TestCase):
    ...
    def test_agent_config_special_chars_do_not_break_resub(self) -> None:
        ...
        # agent id is unlikely, but a backslash in an agent string is realistic
        ...
        # NOTE: the release line serializes only primary/fallback per task (no
        # `model:` line — that key is a main-only agentConfig feature), so the
        # non-ASCII payload rides on `fallback` here.
        config = self._default_config()
        config["agentConfig"] = {
            "defaultPrimary": "claude",
            "defaultFallback": "codex",
            "perTask": {"dev": {"primary": r"agent\g<0>", "fallback": "opus — żółw"}},
        }
