...
class SprintStatusDoneInTextTests(unittest.TestCase):
    ...
    def test_descriptive_slug_wins_over_bare_prefix_regardless_of_order(self) -> None:
        ...
        # same story, the descriptive slug must win deterministically (the
        # prefix resolver only matches `1-1-<slug>` keys, skipping a bare
        # `1-1:` row), not whichever comes first.
